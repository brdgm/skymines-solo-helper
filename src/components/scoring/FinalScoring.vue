<template>
  <p v-html="t('endOfGame.scoring.introduction')"></p>
  ...
  computed: {
    totalCoins() : number[] {
      ...
      for (let i=0; i<this.playerCount+this.botCount; i++) {
        result[i] = (this.coins[i] ?? 0)
            + ((this.astrogoShares[i] ?? 0) * (this.astrogoShareValue ?? 0))
            + ((this.tawacShares[i] ?? 0) * (this.tawacShareValue ?? 0))
            + ((this.skymineShares[i] ?? 0) * (this.skymineShareValue ?? 0))
            + ((this.minervaShares[i] ?? 0) * (this.minervaShareValue ?? 0))
            + (this.heliumCoins[i] ?? 0)
            + (this.researchCoins[i] ?? 0)
      }
      return result
    }
  },
